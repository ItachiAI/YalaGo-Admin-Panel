...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">{{trans('lang.moving_vehicles')}}</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{url('/dashboard')}}">{{trans('lang.dashboard')}}</a></li>
                <li class="breadcrumb-item active">{{trans('lang.moving_vehicles')}}</li>
            </ol>
        </div>
    </div>
                        ...
                        <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
                            <li class="nav-item">
                                <a class="nav-link active" href="{!! url()->current() !!}"><i
                                            class="fa fa-list mr-2"></i>{{trans('lang.moving_vehicles')}}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="{{route('moving-vehicles.save')}}"><i
                                            class="fa fa-plus mr-2"></i>{{trans('lang.moving_vehicle_add')}}</a>
                            </li>
                        </ul>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive m-t-10">
                            <table id="movingTable"
                                ...
                                <thead>
                                <tr>
                                    <?php if (in_array('moving.delete', json_decode(@session('user_permissions')))) { ?>
...
<script type="text/javascript">
    ...
    var ref = database.collection('moving_vehicle');
    ...
    if ($.inArray('moving.delete', user_permissions) >= 0) {
    ...
    $("#deleteAll").click(function () {
        if ($('#movingTable .is_open:checked').length) {
            if (confirm(deleteSelectedRecordMsg)) {
                jQuery("#overlay").show();
                $('#movingTable .is_open:checked').each(function () {
                    var dataId = $(this).attr('dataId');

                    database.collection('moving_vehicle').doc(dataId).delete().then(function () {
    ...
    $(document).on("click", "input[name='isActive']", function (e) {
        ...
        var id = this.id;
        if (ischeck) {
            database.collection('moving_vehicle').doc(id).update({'enable': true}).then(function (result) {

            });
        } else {
            database.collection('moving_vehicle').doc(id).update({'enable': false}).then(function (result) {
    ...
    $(document).on("click", "a[name='moving-vehicle-delete']", function (e) {
        if (confirm(deleteMsg)) {
            var id = this.id;
            jQuery("#overlay").show();
            database.collection('moving_vehicle').doc(id).delete().then(function (result) {
