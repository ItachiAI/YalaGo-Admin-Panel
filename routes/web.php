...
Route::middleware(['permission:moving,moving.list'])->group(function () {

    Route::get('/moving-vehicles', [App\Http\Controllers\movingVehicleController::class, 'index'])->name('moving-vehicle');
});

Route::middleware(['permission:moving,moving.' . ((str_contains($_SERVER['REQUEST_URI'], 'save')) ? (explode("save", $_SERVER['REQUEST_URI'])[1] ? "edit" : "create") : $_SERVER['REQUEST_URI'])])->group(function () {

    Route::get('/moving-vehicles/save/{id?}', [App\Http\Controllers\movingVehicleController::class, 'save'])->name('moving-vehicles.save');
});
